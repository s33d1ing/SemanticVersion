...
Describe "SemanticVersion api tests" -Tags "CI" {
    ...
    Context "Comparisons" {
        ...
        It "Equality <operand>" -TestCases @(
            ...
            @{ operand = [SemanticVersion]::new(1, 0, 0, "alpha.1.1") }
        ) {
            param($operand)
    ...
    Context "Error handling" {
        ...
        It "Negative version arguments" {
            { [SemanticVersion]::new(-1, 0) } | Should -Throw -ErrorId "*ArgumentException*"
            { [SemanticVersion]::new(1, -1) } | Should -Throw -ErrorId "*ArgumentException*"
            { [SemanticVersion]::new(1, 1, -1) } | Should -Throw -ErrorId "*ArgumentException*"
        }

        It "Incompatible 'Version' throws" {
            # Revision isn't supported
            { [SemanticVersion]::new([Version]::new(0, 0, 0, 4)) } | Should -Throw -ErrorId "*ArgumentException*"
            { [SemanticVersion]::new([Version]::new("1.2.3.4")) } | Should -Throw -ErrorId "*ArgumentException*"
        }
    }

    Context "Serialization" {
        $testCases = @(
            @{ errorId = "*ArgumentException*"; expectedResult = "1.0.0"; semver = [SemanticVersion]::new(1, 0, 0) }
            @{ errorId = "*ArgumentException*"; expectedResult = "1.0.1"; semver = [SemanticVersion]::new(1, 0, 1) }
            @{ errorId = "*ArgumentException*"; expectedResult = "1.0.0-alpha"; semver = [SemanticVersion]::new(1, 0, 0, "alpha") }
            @{ errorId = "*ArgumentException*"; expectedResult = "1.0.0-Alpha-super.3+BLD.a1-xxx.03"; semver = [SemanticVersion]::new(1, 0, 0, "Alpha-super.3+BLD.a1-xxx.03") }
        )
    ...
    Context "Formatting" {
        It "Should not throw when default format-table is used" {
            ...
            { [SemanticVersion]::new(7, 0, 0, "preview.4") | Format-Table | Out-String } | Should -Not -Throw
        }
    }
}
